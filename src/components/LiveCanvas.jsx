...
import { RefreshCw, Zap, Palette, Type, Code2, Sparkles, Check, MousePointer2, ShieldCheck, ArrowRight, Star } from 'lucide-react';
      ...
      return (
        ...
        >
          <button
            ...
            className="px-8 py-3.5 font-bold text-sm select-none transition-all duration-200 pointer-events-none hover:scale-105"
          >
            {el.text}
          </button>
        </div>
    ...
    >
      ...
      <div className="w-full flex items-center justify-between px-4 py-2 bg-slate-900/80 border-b border-slate-800 shrink-0 z-20 sticky top-0 left-0 backdrop-blur-xl">
        ...
        <div className="text-[11px] font-bold text-slate-400 flex items-center gap-2 font-mono">
          ...
          {activeFullPageTemplate ? 'Full A-Z Website Template Mode' : `Freeform Stage (${elements.length} Nodes)`}
        </div>
      ...
      {activeFullPageTemplate ? (
        <div className="flex-1 w-full bg-slate-950 text-white p-6 space-y-12 overflow-y-auto">
          ...
          <nav className="w-full flex items-center justify-between px-6 py-4 bg-slate-900/60 border border-slate-800/80 rounded-2xl backdrop-blur-xl">
            <span className="text-base font-black text-white font-heading">{activeFullPageTemplate.navbar?.logo}</span>
            <div className="flex items-center gap-6 text-xs text-slate-300 font-medium">
              {activeFullPageTemplate.navbar?.links.map((link, i) => (
                <span key={i} className="hover:text-cyan-400 transition cursor-pointer">{link}</span>
              ))}
            </div>
            <button className="px-5 py-2 bg-gradient-to-r from-cyan-500 to-purple-600 rounded-xl text-xs font-bold text-white shadow-lg shadow-cyan-500/20 hover:scale-105 transition">
          ...
          <section className="text-center py-12 px-4 space-y-6 max-w-4xl mx-auto">
            ...
            <div className="flex flex-wrap items-center justify-center gap-3 pt-4">
              <button className="px-8 py-3.5 rounded-2xl bg-gradient-to-r from-cyan-500 to-purple-600 text-white font-bold text-sm shadow-xl shadow-cyan-500/25 hover:scale-105 transition flex items-center gap-2">
                <span>{activeFullPageTemplate.hero?.primaryBtn}</span>
                <ArrowRight className="w-4 h-4" />
              </button>
              <button className="px-7 py-3.5 rounded-2xl bg-slate-900 border border-slate-800 text-slate-300 hover:text-white hover:border-slate-700 text-sm font-bold transition">
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-5xl mx-auto">
              {activeFullPageTemplate.features?.map((feat, i) => (
                <div key={i} className="p-6 bg-slate-900/60 border border-slate-800/80 rounded-2xl space-y-3 hover:border-cyan-500/40 transition">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
                {activeFullPageTemplate.pricing.map((plan, i) => (
                  ...
                  >
                    ...
                    <button className={`w-full py-3 rounded-2xl font-bold text-xs transition ${
                      plan.highlight 
