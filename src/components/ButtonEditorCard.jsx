...
import { Check, Copy, Code2, Play, RotateCcw, Tag } from 'lucide-react';
  ...
  return (
    <div className="bg-slate-900/50 border border-slate-800/90 hover:border-indigo-500/40 rounded-2xl overflow-hidden flex flex-col shadow-xl transition-all duration-300 group">
      ...
      <div className="p-5 flex-1 flex flex-col bg-[#12141c]/90">
        <div className="flex justify-between items-start mb-3">
          ...
          <div className="flex space-x-1.5 shrink-0 ml-2">
            ...
            >
              ...
              <span>{isExpanded ? 'Hide Editor' : 'Edit Code'}</span>
            </button>
        ...
        {/* Expandable Live Editor (Shrink/Expand feature) */}
        {isExpanded && (
          <div className="mt-3 space-y-3 pt-3 border-t border-slate-800 animate-in fade-in slide-in-from-top-2 duration-200">
            <div className="flex justify-between items-center">
              ...
              {isModified && (
                <button 
                  onClick={handleReset}
                  className="flex items-center text-[10px] text-slate-400 hover:text-amber-400 transition-colors"
                >
            ...
            <textarea 
              className="w-full bg-[#07080c] border border-slate-800 rounded-lg p-2.5 text-xs font-mono text-orange-300 focus:outline-none focus:border-indigo-500 custom-scrollbar resize-y min-h-[70px]"
              value={htmlCode}
              onChange={(e) => setHtmlCode(e.target.value)}
              spellCheck="false"
            />

            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Live CSS</span>
            <textarea 
              className="w-full bg-[#07080c] border border-slate-800 rounded-lg p-2.5 text-xs font-mono text-blue-300 focus:outline-none focus:border-indigo-500 custom-scrollbar resize-y min-h-[110px]"
              value={cssCode}
